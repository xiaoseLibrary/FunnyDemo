      ...
      >
        <div :class="{ node: true, hasMate: treeData.mate }">
          ...
          >
            <div
              class="avat"
              @click="tiggkeImg(treeData)"
              :class="[treeData.bigImg ? '' : 'bigImg']"
            >
              <img :src="treeData.bigImg ? treeData.image_url : imgsize" />
            </div>
            <div class="name" @click="$emit('click-node', treeData, null)">
              {{ treeData.name }}
            </div>
          </div>
          <template v-if="Array.isArray(treeData.mate) && treeData.mate.length">
            ...
            >
              ...
              <div class="name" @click="$emit('click-node', mate)">
                {{ mate.name }}
              </div>
            </div>
          </template>
...
export default {
    ...
    json: {
      handler(Props) {
        let extendKey = function(jsonData) {
          ...
          jsonData.bigImg =
            jsonData.bigImg === void 0 ? true : !!jsonData.bigImg;
          if (Array.isArray(jsonData.children)) {
            jsonData.children.forEach(c => {
              c.parent_name = Props.name;
              extendKey(c);
            });
          }
          return jsonData;
  ...
  methods: {
    ...
    tiggkeImg(treeData) {
      ...
      treeData.bigImg = !treeData.bigImg;
      this.$forceUpdate();
    }
  }
};
